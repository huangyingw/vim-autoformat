...
if !exists("g:formatprg_args_expr_java")  && !exists("g:formatprg_args_java")
  ...
  if s:hostname == "huangyingw-uuid.local"
    let g:formatprg_args_expr_java = '"--mode=java -fxejoO  -t2 --style=java -pcH".(&expandtab ? "s".&shiftwidth : "t")'
  else
    let g:formatprg_args_expr_java = '"--mode=java -fxejoO -y  -t2 --style=java -pcH".(&expandtab ? "s".&shiftwidth : "t")'
  endif
endif
